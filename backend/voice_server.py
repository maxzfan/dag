    ...
    try:
        ...
        if orchestrator_state.get("pending_questions"):
            ...
            if brief:
                ...
                # Start Detail with no prior answers but instruct to avoid redundant questions
                seed_context = ("User message: " + user_text + "\n" +
                                "If you need more info, ask one concise question at a time. Do not repeat questions.")
                follow_up, spec = detail_run(
                    brief,
                    seed_context,
                    None,
                ...
                # If follow_up repeats, try to generate a different one
                if follow_up and follow_up in (orchestrator_state.get("asked_questions") or []):
                    alt_context = seed_context + "\nPreviously asked questions:\n" + "\n".join(orchestrator_state.get("asked_questions") or []) + "\nAsk a different question that was not asked before."
                    alt_follow, alt_spec = detail_run(
                        brief,
                        alt_context,
                        None,
